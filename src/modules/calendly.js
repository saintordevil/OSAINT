// Calendly booking link parser.

import { compactData, extractMeta, extractTitle, fetchHtml, looksLikeReservedSlug, normalizeUrl, pathParts } from './_helpers.js';

export default async function calendly(url) {
    try {
        const parsed = normalizeUrl(url, 'https://calendly.com');
        if (!parsed || parsed.hostname.toLowerCase() !== 'calendly.com') return { error: 'Invalid Calendly booking URL' };

        const parts = pathParts(parsed);
        const owner = parts[0];
        if (!/^[A-Za-z0-9_-]{2,64}$/.test(owner || '') || looksLikeReservedSlug(owner)) {
            return { error: 'Invalid Calendly booking URL' };
        }

        const data = { username: owner, profile_url: `https://calendly.com/${owner}`, booking_id: parts[1], share_type: 'booking-link' };
